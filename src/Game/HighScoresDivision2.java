 ...
 *
 */
public class HighScoresDivision2 {
	...
	public static void work() {
		...
		try {
			readFile("HighScoresDivision2.txt");
		}catch (IOException e) {
			
		}
		
		ProblemTypeScreen p = new ProblemTypeScreen();
		p.setVisible(false);
		type = p.getInt();
		p.dispose();
		
		Division d = new Division();
		secs = (double) d.getDouble();
		d.setVisible(false);
		d.dispose(); 
		...
		JLabel info = new JLabel("HIGHSCORES - THESE ARE THE FASTEST TIMES FOR DIVISION DIFFICULTY 2!");
		m.gridx = 1;
		m.gridy = 0;
		panel.add(info, m);
	...
	public static void writeFile() throws IOException {
		PrintWriter pw = new PrintWriter(new FileWriter("HighScoresDivision2.txt"));
